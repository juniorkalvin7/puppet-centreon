Puppet::Type.type(:centreon_service).provide(:clapi) do
  ...
  def check_command_arguments
    output = `grep "SERVICE;setparam;#{@resource[:hostname]};#{@resource[:description]};check_command_arguments;#{@resource[:check_command_arguments]}" #{@resource[:clapi_export_file]}`
    if output.to_s.lines.count >= 1
      @resource[:check_command_arguments]
    else
      :absent
    end
  end
